...
本文记录 BOOX P6 小白马在 BOOX OS 4.1 上通过 EDL 写入 Magisk 修补 boot，并修复 Magisk 管理器打不开的问题。
...
- 系统版本一致：直接使用“快速开始”脚本。
- 系统版本不同：按“手动流程”重新提取并修补当前固件自己的 boot 和 `services.jar`。
...
`p6_boot_a.img.xz` 和 `p6_boot_a_magisk.img.xz` 是压缩后的 boot 镜像。需要手动解压时：

```bash
xz -dk artifacts/p6_boot_a.img.xz
xz -dk artifacts/p6_boot_a_magisk.img.xz
```

手动准备 EDL 工具时，把 [bkerler/edl](https://github.com/bkerler/edl) 放在仓库同级或仓库内均可。快速脚本会自动 clone 到仓库内的 `edl/`：
...
系统版本不一致、需要自制 boot/AMS 模块，或想理解完整过程时按本节操作。

本节命令默认在仓库根目录下准备文件，并在需要运行 `edl.py` 时进入 `edl/`。因此 boot 镜像路径通常写成 `../p6_boot_a.img`。
...
```bash
adb install -r artifacts/Magisk-v30.7.apk
```
